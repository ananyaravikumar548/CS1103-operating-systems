...
search_file_name() {
    read -p "Enter a file name to search: " name
    echo "Searching for files with name '$name'..."
    # Use ls, grep, and sort in a pipeline
    ls -l | grep "$name" | sort
    echo ""
}
...
echo "3. Search for a file by name"
echo "4. Count the number of files"
echo "5. Exit"
