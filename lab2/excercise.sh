...
task_pid=$! # Captures the PID of the background task
...
echo -e "\nPausing the background task..."
kill -STOP $task_pid
#pausing the background manually 
echo -e "use 'kill -STOP $task_pid' command to pause manually"
echo -e "use 'ps -o pid,stat,cmd -p $task_pid' to check the status"
...
echo -e "\nResuming the background task..."
kill -CONT $task_pids
#resuming it manually
echo -e "use the command 'kill-CONT $task_pid to resume the background manually"
echo -e "use 'ps -o pid,stat,cmd -p $task_pid' to check the status"
read -p "press enter to continue"
